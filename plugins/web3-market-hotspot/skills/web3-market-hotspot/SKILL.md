...
💡 创作者怎么用（按角色拆分：思路 + 框架 + 今日示例）
  👤 推特 KOL：观点驱动/短平快/敢说/造冲突
   · 框架：Hook(反常识钩子) → 数据(1-2硬数字) → 一句判断 → 互动CTA
   · 今日示例：<一句话>
  👤 小红书 KOL：精致图文/反差干货/避坑，封面大字钩子
   · 框架：封面钩子 → 标题 → 正文(痛点→数据→观点→引导收藏) → 标签+时段
   · 今日示例：<一句话>
  👤 TikTok KOL：前3秒抓人/口播+画面/悬念开头带数字
   · 框架：前3秒钩子 → 30秒口播(钩子→讲什么→反转) → 引导关注/评论
   · 今日示例：<一句话>
  👤 Web3/美股社区创作者：偏深/研究向/讲生态与事件影响
   · 框架：事件是什么 → 为什么(链上/数据佐证) → 影响谁(生态/代币) → 机会与风险
   · 今日示例：<一句话>
  👤 平台话题运营：造话题/蹭热点做聚合榜单/驱动UGC
   · 框架：挑热点词 → 定话题名 → 给内容角度(引导发帖) → 活动/激励 → 追踪数据
   · 今日示例：<一句话>
  👤 平台社区运营：促活/答疑/引导讨论/沉淀精华
   · 框架：固定栏目(今日快评) → 抛问题引导互动 → 归纳答疑 → 精华沉淀 → 达人激励
   · 今日示例：<一句话>
  👤 平台直播运营：脚本/节奏/互动设计/道具(数据卡图表)
   · 框架：开场钩子(10s定主题) → 主线分段(每段观点/热点+数据) → 互动(投票/提问) → 收尾CTA
   · 今日示例：<一句话>

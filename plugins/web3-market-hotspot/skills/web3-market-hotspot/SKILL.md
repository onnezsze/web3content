...
💡 创作者怎么用（通用框架 · 不绑定热点）
  ...
  给内容创作者/运营的提示词是**通用可复用框架**，不针对任何具体热点。完整 7 角色框架见 `references/creator-frameworks.md`：
  用法 → 复制你要用的那个角色框架 → 把 `{ }` 占位符替换成你选定的热点内容 → 整段发给任意 AI（Claude/GPT/Hermes）即可产出脚本。框架长期可用，热点随你填。
  占位符：{主题} {硬数据} {新闻标题} {原声} {角度}（含义见 references/creator-frameworks.md 顶部）。
  7 个角色：推特KOL / 小红书KOL / TikTokKOL / Web3·美股社区创作者 / 平台话题运营 / 平台社区运营 / 平台直播运营。
  快速示例（以推特 KOL 为例，其余角色见框架文件）：
  ```txt

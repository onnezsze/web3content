...
🔍 圈内动态
  取 5 条，每条 = 标题 + 内容摘要 + 信息来源。取自 report.py 的「圈内动态」或按主体在新闻/社媒/宏观里定向捞取。
  监测主体（不写进标题）：交易所(上/下架·公告·新合约)、链·生态(主网·升级·安全事件·TVL·空投)、交易所老板高管(CZ/Coinbase CEO)、孙哥(孙宇晨/波场)、币圈KOL(名人·巨鲸·观点)、特朗普(加密相关)。
  筛选与排序优先级：孙哥 > 特朗普 > 交易所 > 老板高管(如CZ) > 币圈KOL > 链/生态，同级再按重要程度。
  1. <一句话标题>
  · 摘要：<一句话>
  · 来源：<媒体>

💡 创作者怎么用（按角色 · 整段复制给 AI 即可出脚本）
  每个角色输出一个「复制即用」代码块：角色定位 + 【今日素材线索】 + 【创作框架】 + 【输出格式】。
  创作者整段复制 → 丢给 AI → 得创作脚本。7 个角色：推特KOL / 小红书KOL / TikTokKOL / Web3·美股社区创作者 / 平台话题运营 / 平台社区运营 / 平台直播运营。
  【今日素材线索】必须"厚"够用，至少含：② 关键硬数据（行情/涨跌/资金/异动，多个）；② 2-3 条真实新闻标题（可直接引用）；③ 1-2 条社区原声（真实引用，如"……"）；④ 2-3 个内容角度/钩子备选。
  结构示范（以推文为例）：
  ```txt
  你是推特加密货币 KOL，风格犀利敢下判断。基于今日素材线索产出观点推文：
  【今日素材线索】
  · 市场：BTC $80,856（24h +4.64%/7d +25.7%）5 月来首破 8 万；ETH…；SOL…
  · 资金：ETF 单周净流入 $2.6B；资金费率 10.95%；空头爆仓 $220M
  · 新闻：CoinDesk《…回调才关键》；TheBlock《ETF 10 月最强》
  · 原声：抄底"不要再跌了…"；看多"日线买点突破…"
  · 冲突：牛市确认 vs 资金泡沫
  【创作框架】反常识钩子 → 数据(1-2硬数字) → 一句判断 → 互动 CTA
  【输出格式】① ≤140 字观点 ② 3 节 Thread ③ 一个互动问题
  ```

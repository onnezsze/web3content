...
面向 Web3 / 美股内容创作者与圈内人：**3 分钟抓住核心热点、理解来龙去脉、直接开写**。每个热点**不用固定标签词**，用一段自然流畅的文字把要素写进去 —— 讲什么、来龙去脉(前因→进展→影响)、创作者视角(钩子·角度·情绪)、交易信号与风险；创作者提示词框架附文档末尾。来源只在文末统一标注，正文不刷来源噪音。
**每周一**：同时交付**两份文档** —— ① 日报（今日内容简报）② 周报（本周回顾 + 下周可预估热点）；两者均按此自然段式，框架附各自文末。
...
🔥 今日核心看点（3–5 条 · 自然段式）
  #1 《一句话标题》
   <1-3 句自然段：讲什么 → 前因/进展/影响(脉络) → 创作者视角(钩子·角度·情绪) → 交易信号与风险。要素融入行文，不列标签。>
  #2 … #5 …
...
📌 今日要闻（最多 5 条，每条 1 句话 + 附来源链接，去噪）
  ...
  同优先级再按重要程度；优先取 report.py 输出的「今日要闻候选(按优先级)」（已带 url）。每条要闻**必须附来源链接**。
  · <一句话要闻> https://…

🔍 圈内动态（5 条 · 自然段式，仅当天最新）
  只取**当天最新**；社媒(币安广场)仅当命中新事件信号词(上线/下架/公告/爆仓/起诉/监管等)才视为新动态，排除旧瓜回味。旧闻有新进展的在行文里讲清前因后果。
  监测主体优先级：孙哥(含八卦·最高，上限 2) > 特朗普(加密相关) > 交易所 > 老板高管 > 币圈大V·名人 > 链/生态。
  1. <一句话标题>
   <1-3 句自然段：事件讲什么 → 来龙去脉 → 创作者视角 → 交易信号/风险；末尾注来源(媒体)。>（来源：<媒体>）
...
🕐 周度回顾（周一简报专属 · v7.7 新增）
  ...
  ② 下周可预估热点：两类，每条带**明确时间节点**，不做模棱两可：
     ● 确定性事件：已锁定日期、大概率会发生（宏观日历/非农·CPI·FOMC·财报/项目升级·解锁·TGE/活动等），给具体日期。
     ● 争议应验：当前或上周有**明显争议**的热点，下周可能见分晓（牛熊争论/关键涨跌位/关键人物多空/监管动向），给"应验看点"。
     · 优先级：高（美联储/宏观数据/美股明星财报）> 中（BTC/ETH/SOL/MEME 主流币）> 低（其他加密/美股/日韩/港股/A股·创业板·科创板）。
  格式（每条四要素）：
  📅 【上周回顾】／【本周预估】
   1. <日期，如 8/21> <一句话标题>
    · 摘要：<一句话讲清事件>
  ...
  （简报中**每个热点**均已按素材包标准交付：{主题} {硬数据} {新闻标题} {原声} {角度}，占位符含义见 references/creator-frameworks.md 顶部。）

💡 创作者怎么用（分两类视角）
  创作者提示词框架**直接附在文档末尾**（见 references/creator-frameworks.md；占位符 {主题}/{硬数据}/{新闻标题}/{原声}/{角度} 与简报里每个热点字段一一对应）。
  **两类视角，能力要求不同**：
  · 个人创作者（推特/小红书/TikTok/Web3美股社区）：重**观点 / 内容价值 / 流量**，拒绝总结性复述，必须有态度、钩子、能被记住的记忆点。
  · 平台运营（话题/社区/直播/内容运营）：面向平台**交易用户 / 热点消费用户**，讲清事件**前因后果**、拆出**深层传播逻辑**、并**关联对用户的交易信号**（资金/情绪/链上/节奏）。
  用法：从简报挑一个热点（已是素材包标准）→ 复制文末对应角色框架 → 把该热点的五字段填入 → 一块发给任意 AI（Claude/GPT/Hermes）即产出，无需自己加工热点。

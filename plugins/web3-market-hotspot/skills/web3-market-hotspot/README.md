...
- 两块，每条含 **日期 / 标题 / 摘要 / 关键数据** 四要素：
  - ① **上周热点回顾**：复盘最近 7 天重要热点事件。取自 report.py **独立检索近 7 天新闻**的候选（日期 + 标题 + 摘要 + 跨源数），非每日 top3 存档。
  - ② **本周可预估热点**：当周已知/大概率将发生的事件（财经日历 / 听证会 / 代币解锁 / 主网升级 / 发布会 / 财报 / CPI·非农·议息 / 交割·到期 / TGE·空投等）。
- report.py 新增 `weekly_review()`：**独立检索近 7 天新闻**（三 RSS + 东财 + 华尔街见闻）按时间窗口 + 跨源数聚合出「上周热点」候选（**非每日 top3 存档，避免丢失**）；按前瞻关键词扫描新闻/社媒/宏观形成「本周可预估热点」候选；仅 `weekday()==0`（周一）时输出「周度回顾」段。

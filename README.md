...
- 两块，每条含 **日期 / 标题 / 摘要 / 关键数据** 四要素：
  - ① **上周热点回顾**：复盘最近 7 天重要热点事件。取自 report.py 输出的近 7 天每日 top3 候选（日期 + 标题 + 评分）。
  - ② **本周可预估热点**：当周已知/大概率将发生的事件（财经日历 / 听证会 / 代币解锁 / 主网升级 / 发布会 / 财报 / CPI·非农·议息 / 交割·到期 / TGE·空投等）。
- report.py 新增 `weekly_review()`：读 hot_history.json 近 7 天每日 top3 形成「上周热点」候选；按前瞻关键词扫描新闻/社媒/宏观形成「本周可预估热点」候选；仅 `weekday()==0`（周一）时输出「周度回顾」段。
